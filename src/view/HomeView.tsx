...
import { Github, Book, Mail, Twitter } from "lucide-react";
...
function getSocialLinks(): { href: string, icon: JSX.Element }[] {
  const { GithubUsername, BlogUrl, EmailUrl, TwitterUrl, BilibiliUid, QqId } = config
  const result = []
  if (GithubUsername) {
    result.push({
  ...
  if (BilibiliUid) {
    result.push({
      href: `https://space.bilibili.com/${BilibiliUid}`,
      icon: <BilibiliIcon />,
    })
  }
  ...
  if (QqId) {
    result.push({
      href: `https://wpa.qq.com/msgrd?v=3&site=qq&menu=yes&uin=${QqId}`,
      icon: <QqIcon />,
    })
  }
